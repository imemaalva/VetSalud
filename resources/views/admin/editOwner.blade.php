<x-app-layout>
    ...
    <form class="px-24 my-4" method="GET" action="{{ route('admin.updateOwner', $dueno->id) }}">
        @csrf
        <div class="mt-2">
            <label for="nombre" class="text-sm text-gray-500">Nombre</label>
            <input id="nombre" name="nombre" type="text" value="{{ $dueno->nombre }}" required
        ...
        <div class="mt-2">
            ...
            <input id="email" name="email" type="text" value="{{ $dueno->email }}" required
                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm sm:w-full sm:border-t-0 sm:border-b-2 sm:border-indigo-300">
        </div>
        <div class="mt-2">
            <label for="telefono" class="text-sm text-gray-500">Teléfono</label>
            <input id="telefono" name="telefono" type="number" value="{{ $dueno->telefono }}" required
                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm sm:w-full sm:border-t-0 sm:border-b-2 sm:border-indigo-300">
        </div>
        <div class="mt-2">
            <label for="direccion" class="text-sm text-gray-500">Dirección</label>
            <input name="direccion" id="direccion" type="string" value="{{ $dueno->direccion }}" required
                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm sm:w-full sm:border-t-0 sm:border-b-2 sm:border-indigo-300">
        </div>
        <div class="flex items-center md:py-5 border-t border-gray-200 rounded-b dark:border-gray-600">
            ...
            <button data-modal-hide="default-modal" type="button"
                ...
                <a href="{{ route('admin.showOwners') }}">Cancelar</a>
            </button>
        </div>
    </form>
</x-app-layout>
